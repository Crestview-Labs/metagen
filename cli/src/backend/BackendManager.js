...
import { ensureProfileDirs, getProjectRoot, fileExists, getAmbientHome } from '../utils/paths.js';
import { ProcessManager } from './ProcessManager.js';
import { ensureUv } from '../utils/uv.js';
export class BackendManager extends EventEmitter {
    ...
    async start(options = {}) {
        ...
        try {
            ...
            this.processHandle = await this.processManager.spawn(pythonPath, ['main.py', '--port', availablePort.toString()], {
                cwd: projectRoot,
                env,
    ...
    async checkDependencies() {
        ...
        if (!await fileExists(pythonPath)) {
            throw new BackendError('Python environment not set up. Run "ambient setup" first.');
        }
        // Check for main.py
