 ...
 */
function getUvBinaryName(): string {
 ...
 */
async function downloadUv(targetPath: string): Promise<void> {
...
export async function ensureUv(): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      'Failed to download uv. You can install it manually:\n' +
 ...
 */
export async function checkUv(): Promise<{ available: boolean; path?: string }> {
  // Check system PATH
  try {
    const systemUv = await which('uv');
 ...
 */
export async function setupPythonEnvironment(uvPath: string, pythonVersion: string = '3.12', force: boolean = false): Promise<string> {
  const venvPath = path.join(getAmbientHome(), 'venv');
  
  // Check if venv already exists
  if (await fileExists(path.join(venvPath, 'bin', 'python')) && !force) {
    console.log(chalk.green('✓ Python environment already exists'));
    return venvPath;
  }
  
  // Remove existing venv if force is true
  if (force && await fileExists(venvPath)) {
    console.log(chalk.yellow('Removing existing Python environment...'));
