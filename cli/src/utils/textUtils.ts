 ...
 */
export function cpCharAt(str: string, index: number): string | undefined {
  const codePoints = [...str];
 ...
 */
export function cpIndexOf(str: string, searchStr: string, fromIndex = 0): number {
  const codePoints = [...str];
 ...
 */
export function isWordBoundary(str: string, pos: number): boolean {
  const codePoints = [...str];
  if (pos <= 0 || pos >= codePoints.length) return true;
 ...
 */
export function findNextWordBoundary(str: string, pos: number): number {
  const codePoints = [...str];
  let i = pos;
  
  // Skip current word characters
  while (i < codePoints.length && /\w/.test(codePoints[i])) i++;
  
  // Skip whitespace
  while (i < codePoints.length && /\s/.test(codePoints[i])) i++;
 ...
 */
export function findPrevWordBoundary(str: string, pos: number): number {
  const codePoints = [...str];
  let i = pos - 1;
  
  // Skip whitespace
  while (i >= 0 && /\s/.test(codePoints[i])) i--;
  
  // Skip word characters
  while (i >= 0 && /\w/.test(codePoints[i])) i--;
