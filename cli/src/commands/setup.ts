...
export const setupCommand = new Command('setup')
  ...
  .option('--python <version>', 'Python version to use', '3.13')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const venvPath = await setupPythonEnvironment(uvPath, options.python, options.force);
...
export async function promptSetup(): Promise<void> {
  ...
  console.log('  • Create Python 3.13 environment');
