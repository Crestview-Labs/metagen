...
async def test_db_engine(tmp_path: Path) -> AsyncGenerator[DatabaseEngine, None]:
...
@pytest_asyncio.fixture
async def robust_backend(test_db_engine: DatabaseEngine) -> AsyncGenerator[SQLiteBackend, None]:
    """Create a backend with robustness features enabled."""
    backend = SQLiteBackend(test_db_engine)
...
class TestCrashRecovery:
    ...
    async def test_database_consistency_after_interrupt(self, tmp_path: Path) -> None:
        ...
        backend1 = SQLiteBackend(db_engine1)
        ...
        if backend1.db_engine:
            await backend1.db_engine._async_engine.dispose()

        # Open new backend and check consistency
        db_engine2 = DatabaseEngine(db_path)
        await db_engine2.initialize()
        backend2 = SQLiteBackend(db_engine2)
    ...
    async def test_wal_checkpoint_recovery(self, robust_backend: SQLiteBackend) -> None:
        ...
        assert robust_backend.db_engine is not None
        async with robust_backend.db_engine._async_engine.begin() as conn:
...
class TestDataIntegrity:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="SQLModel with Pydantic v2 is very lenient with type coercion")
    async def test_data_type_validation(self, robust_backend: SQLiteBackend) -> None:
        ...
        # SQLModel with Pydantic v2 is more lenient with type coercion
        # and many fields have defaults. Let's test enum validation instead
