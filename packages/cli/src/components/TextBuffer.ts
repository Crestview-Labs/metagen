...
export function useTextBuffer({
  ...
  const resetBuffer = useCallback(
    (): void => {
      dbg('resetBuffer - clearing all state');
      setLines(['']);
      setCursorRow(0);
      setCursorCol(0);
      setPreferredCol(null);
  ...
  const openInExternalEditor = useCallback(
    async (opts: { editor?: string } = {}): Promise<void> => {
      ...
      const tmpDir = fs.mkdtempSync(pathMod.join(os.tmpdir(), 'metagen-edit-'));
...
export interface TextBuffer {
  ...
  /**
   * Completely resets the buffer state, clearing undo/redo history.
   * Use this to prevent state corruption after multiple exchanges.
   */
  resetBuffer: () => void;
  /**
